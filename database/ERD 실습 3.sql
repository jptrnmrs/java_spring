...
insert into `Products` values (120103, 12 ,10010 ,'바로구이 부채살 250g'			,  null	,  21000,  61, 0);
...
insert into `OrderItems` values (1,'22010210001',100101,   25000, 20, 1);
insert into `OrderItems` values (1,'22010210002',120103,   21000, 10, 3);
insert into `OrderItems` values (1,'22010310001',130112,   15000,   0, 1);
insert into `OrderItems` values (1,'22010310001',130101,   56000,   0, 2);
insert into `OrderItems` values (1,'22010210010',110101,   76000,   5, 1);
insert into `OrderItems` values (1,'22010310100',160103, 120000,   0, 1);
insert into `OrderItems` values (1,'22010410101',170115, 900000, 12, 1);
insert into `OrderItems` values (1,'22010510021',110101,   76000,   5, 1);
insert into `OrderItems` values (1,'22010510027',130101,   56000,   0, 2);
insert into `OrderItems` values (1,'22010510021',100101,   25000, 20, 1);
insert into `OrderItems` values (1,'22010510031',170115, 900000, 12, 1);
insert into `OrderItems` values (1,'22010710110',120103,   21000, 10, 5);
...
insert into `Carts` values (1,'user6', 130101, 1, '2022-01-10 10:50:12');
...
# 문제4. 모든 주문의 주문번호, 주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오. 단 주문금액에 10만원 이상, 큰 금액순으로 조회,
#			금액이 같으면 이름이 사전순으로 될것
select
	`orderNo`,
    `userId`,
    `userName`,
...
select
    ...
    `prodName`,
    `prodPrice`,
    concat(`prodDiscount`,'%') as `할인율`,
    case
		when `prodDiscount` = 0 then `prodPrice`
		else floor(`prodPrice`-`prodPrice`*(`prodDiscount`/100))
        end as `할인가`
from `Products`;
